...
"""
location_dict = {}
with open(join(DIRPATH, 'json/location.json'), encoding='utf-8') as location_json:
...
for lang in os.listdir(join(DIRPATH , 'json/lang')):
    with open(join(DIRPATH,'json' , 'lang' , lang) , 'r', encoding='utf-8') as f:
...
def end_state(update: Update , context: CallbackContext) ->int:
    ...
    try:
        ...
        risultati = scraper.search_aule_libere(
            sede=location_dict[location],
            giorno=dt_date,
            ora_dal=f"{start_time}:15",
            ora_al=f"{end_time}:15"
        )
        
        messaggio = string_builder.formatta_messaggio_risultati(risultati)
        
        update.message.reply_text(messaggio, parse_mode=ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(initial_keyboard))
        
        donate_keyboard = [[InlineKeyboardButton("☕ Offrimi un caffè", url="https://www.paypal.me/PeteGuidetti")]]
        update.message.reply_text("Ti è stato utile?", reply_markup=InlineKeyboardMarkup(donate_keyboard))
        
        logging.info("%d : %s search was: %s %s %d %d" , user.id , user.username , location , date , start_time , end_time )
    except Exception as e:
        logging.error(f"Exception during search: {e}", exc_info=True)
